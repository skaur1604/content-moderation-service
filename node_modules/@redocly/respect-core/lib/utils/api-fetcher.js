...
import { getVerboseLogs, conditionallyMaskSecrets, findPotentiallySecretObjectFields, } from '../modules/logger-output/index.js';
...
export class ApiFetcher {
    constructor(params) {
        ...
        this.initVerboseResponseLogs = ({ headerParams, host, path, method, body, statusCode, responseTime, responseSize, }) => {
        ...
        this.clearVerboseResponseLogs = () => {
            this.verboseResponseLogs = undefined;
        };
        this.fetchResult = async ({ ctx, step, requestData, workflowId, }) => {
            ...
            if (urlToFetch.startsWith('/')) {
                ctx.options.logger.error(bgRed(` Wrong url: ${inverse(urlToFetch)} `) +
                    ` Did you forget to provide a correct "serverUrl"?\n`);
            }
            const contentType = headers['content-type'] || '';
            if (requestBody && !contentType) {
                ctx.options.logger.error(bgRed(` Incorrect request `) +
            ...
            const workflowLevelXSecurityParameters = ctx.workflows.find((workflow) => workflow.workflowId === workflowId)?.['x-security'] || [];
            ...
            if (lastDigestSecurityScheme) {
                ...
                if (first401Result.status !== 401) {
                    ...
                    throw new StatusCodeError(`Digest auth failed, expected 401 status code, but received ${first401Result.status} in the first response`);
                }
                if (!wwwAuthenticateHeader) {
                    ...
                    throw new UnexpectedError('Digest auth failed, no www-authenticate header received in the first response');
                }
                const { realm, nonce, opaque, qop, algorithm, cnonce, nc } = parseWwwAuthenticateHeader(wwwAuthenticateHeader);
